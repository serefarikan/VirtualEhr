...
sed -i 's^<!--template-->^<!--template-->\n<a href="https://github.com/serefarikan/VirtualEhr/commit/'"$COMMIT_ID"'">'"$COMMIT_SHORT"' (VEHR/Code4Health)</a><span> :  ('"$BUILD_DATE_TIME"')</span> <a href="site_'"$COMMIT_SHORT"'/surefire-report.html">Surefire test reports</a> <a href="cucumber_'"$COMMIT_SHORT"'/overview-features.html">Cucumber test reports</a> <a href="https://travis-ci.org/serefarikan/VirtualEhr/builds/'"$TRAVIS_BUILD_ID"'">Travis CI build</a> <a href="https://api.travis-ci.org/v3/job/'"$TRAVIS_JOB_ID"'/log.txt">Travis CI build log(raw)</a><br>^g' travis-reports/index.html
...
cp ../vehr-integration-tests/target/site/cucumber-reports/cucumber-html-reports -r ./cucumber_$COMMIT_SHORT
git add -A
git config user.email serefarikan@gmail.com
git config user.name ecisreportsarikan
git commit -m"VirtualEHR build"
